...
% Modified August 2023, so that others can use this script in the future
% for new years. Goal is to set up days of model input data for both
% Synechococcus and Picoeukaryotes. 

% now takes input basepath 
%basepath should look like: '\\sosiknas1\Lab_data\MVCO\FCB\MVCO_Jan2020'
...
function setup_days_newyear(basepath)

year2do = str2num(basepath(end-3:end)); 

%file structure setup
beadpath = [basepath filesep 'data/processed/beads/']; 
eukmodelpath = [basepath filesep 'euk_model/dawnstart_inputs/']; 
synmodelpath = [basepath filesep 'syn_model/dawnstart_inputs/']; 
mergedpath0 = [basepath filesep 'data/processed/grouped/merged/']; 
groupedpath = [basepath filesep 'data/processed/grouped/']; 

plotflag = 1; 
solarpath=fullfile(basepath,'syn_model/');

if ~exist(eukmodelpath)
    mkdir(eukmodelpath)
end
if ~exist(synmodelpath)
    mkdir(synmodelpath)
end
...
%empty small bins
eukvolbins = 2.^[-5:1/5:8]; %For Eukwork
synvolbins = 2.^(-5:0.125:2); %spacing for bins for Syn - log2 base
...
eukcellclass = 4;  %cell group to model (1 = syn, 4 & 5 = euks)
syncellclass = 1; 
cellclass = 1; 
   ...
   %
    ...
    if dielhr(1) <= 5 && dielhr(end) == 24 && isempty(find(diff(dielhr) > 2, 1)),  %full day ==> proceed, otherwise just go to next day
        ...
        for count = 1:length(dielind),  %get all cell data
            ...
            synind = find(temp(:,end) == syncellclass);
            eukind = find(temp(:,end) == eukcellclass);
            ...
            syncellvol{count} = cytosub_SSC2vol(temp(synind,5)./beadvol);  %SSC bu values converted to volume
            eukcellvol{count} = cytosub_SSC2vol(temp(eukind,5)./beadvol);  %SSC bu values converted to volume
        ...
        while ~isempty(temp), %every other one matches
            ...
            syncellvol{temp} = [syncellvol{temp}; syncellvol{temp+1}]; %add together cells measured in same hour
            syncellvol = syncellvol([1:temp, temp+2:end]); %omit second in same hour

            eukcellvol{temp} = [eukcellvol{temp}; eukcellvol{temp+1}]; %add together cells measured in same hour
            eukcellvol = eukcellvol([1:temp, temp+2:end]); %omit second in same hour
        ...
        while ~isempty(temp),  %average across 1 skipped hour
            ...
            tempvol{1} = [syncellvol{temp}; syncellvol{temp+1}]; %add together cells measured in surrounding hours
            syncellvol = [syncellvol([1:temp]) tempvol syncellvol([temp+1:end])];

            tempvol{1} = [eukcellvol{temp}; eukcellvol{temp+1}]; %add together cells measured in surrounding hours
            eukcellvol = [eukcellvol([1:temp]) tempvol eukcellvol([temp+1:end])];
        ...
        syn_Vhists = zeros(length(synvolbins),length(syncellvol));
        syn_N_dist = syn_Vhists;

        euk_Vhists = zeros(length(eukvolbins),length(eukcellvol));
        euk_N_dist = euk_Vhists;


        
        for i = 1:length(syncellvol) %calculate observed size distributions
            syn_N_dist(:,i) = histc(syncellvol{i}, synvolbins);  %size distribution
            syn_Vhists(:,i) = syn_N_dist(:,i)/sum(syn_N_dist(:,i));    %normalized size distribution
        end
        %cellsperml = [sum(N_dist)./cellresultsfordiel(:,2)'./cellresultsfordiel(:,3)'];
        syn_cellsperml = [sum(syn_N_dist)./cellresultsfordiel(:,3)'];
        

        for i = 1:length(eukcellvol) %calculate observed size distributions
            euk_N_dist(:,i) = histc(eukcellvol{i}, eukvolbins);  %size distribution
            euk_Vhists(:,i) = euk_N_dist(:,i)/sum(euk_N_dist(:,i));    %normalized size distribution
        end
        %cellsperml = [sum(N_dist)./cellresultsfordiel(:,2)'./cellresultsfordiel(:,3)'];
        euk_cellsperml = [sum(euk_N_dist)./cellresultsfordiel(:,3)'];


        if length(syncellvol)==25
            if isempty(daylog_fcb{w,2}), daylog_fcb(w,:)=[{day}  {'full day!'}];
            else daylog_fcb{w,2}=[daylog_fcb{w,2} '; full day!']; end
        elseif length(syncellvol) <25
        ...
        if dielstarthr == 0,
            ...
            Vhists = syn_Vhists; 
            N_dist = syn_N_dist; 
            cellsperml = syn_cellsperml; 

            eval(['save ' synmodelpath 'day' num2str(day) 'data day synvolbins Edata Vhists N_dist cellsperml dielstarthr'])  %save data for rerunning batch later
       

            volbins = eukvolbins; 
            Vhists = euk_Vhists; 
            N_dist = euk_N_dist; 
            cellsperml = euk_cellsperml; 

            eval(['save ' eukmodelpath 'day' num2str(day) 'data day eukvolbins Edata Vhists N_dist cellsperml dielstarthr'])  %save data for rerunning batch later
...
eval(['save ' solarpath 'setupdays_log.mat daylog_fcb'])
