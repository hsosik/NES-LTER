...
maxvalue = 1e6;
lbins = 12;
bins = 10.^(2:(log10(maxvalue)-2)/(lbins-1):log10(maxvalue));
binc = [mean([bins(1:end-1); bins(2:end)]) maxvalue];
bins(lbins+1:16) = NaN;
bins(lbins+1:16) = NaN;
maxvalue = 100;
minvalue = 1e-6;
bins_rat = logspace(log10(minvalue), log10(maxvalue), 16);
...
ii = find(partialdatmerged(datbins2,2)>1); % & partialdatmerged(datbins2,5)>100);
h = histmulti5([partialdatmerged(datbins2(ii),4),partialdatmerged(datbins2(ii),2)./partialdatmerged(datbins2(ii),5)],[bins; bins_rat]');
p = NaN(1,lbins);
vtrough = p;
ptrough = p;
lower_cut = 8;
if prctile(partialdatmerged(datbins2(ii),2)./partialdatmerged(datbins2(ii),5),5) > 1e-4
    lower_cut = 10;
end
for cc = 1:lbins
    if sum(h(cc,:)) > 0
        [mx1, px1] = max(h(cc,1:lower_cut));
        d = diff(h(cc,px1:end-2));
        up = find(d>0);
        if (isempty(up) | max(d) <= 4) & mx1 >= 5
            mn1 = min(h(cc,px1:end-2));
            pn1 = find(h(cc,px1:end-2) <= max([mn1*1.2 ceil(mx1/20)])); %/10??? 100?
            pn1 = pn1(1)+px1-1;
            vtrough(cc) = binc_rat(pn1)*0.8;
            ptrough(cc) = pn1;
        else
            if mx1 >= 5 & px1 < lower_cut % & ~isempty(up)
                up = up(1)+px1-1;
                [mx2, px2] = max(h(cc,up:end-1));
                px2 = px2+up-1;
                up = find(diff(h(cc,px2:end-2))>0);
                if mx2>=5 & ~isempty(up)
                    up = up(1)+px2-1;
                    [mx3, px3] = max(h(cc,up:end));
                    px3 = px3+up-1;
                    if mx3 >= 5 & mx3>mx2/4
                        px1 = px2;
                        mx1 = mx2;
                ...
                t = px1:px2;
                if length(t) > 2 & mx2 >= 5
                    fitobject = fit(log10(binc_rat(t))', h(cc,t)', 'smoothingspline', 'SmoothingParam', 0.999);
                    %figure, plot(fitobject, log10(binc_rat(t))', h(cc,t)')
                    yest = feval(fitobject, log10(binc_rat(t))');
                    mn1 = max([0 min(yest)]);
                    %pn1 = find(yest(2:end-1)< max([mn1*1.2 mn1+1])); %don't allow the edges
                    pn1 = find(yest(2:end-1)< max([mn1*1.2 ceil(mx2/10)])); %don't allow the edges
                    pn1 = pn1+1;
                    %if cc <= 8
                        vtrough(cc) = binc_rat(t(pn1(1)))*0.8;
                        ptrough(cc) = t(pn1(1));
                    %else
                    %    vtrough(cc) = binc_rat(t(pn1(end)))*0.8;
                    %end
                end
            else
                ptrough(cc) = 6;
                vtrough(cc) = binc_rat(ptrough(cc))*0.8;                
            end
        end
    end
end
...
%n = (min(d1) <= 2 & sum(d1)<5);
n = (min(d1) <= 1 | sum(d1)<5);
%fitobject = fit(log10(binc(t))', log10(vtrough(t))', 'poly1');
%c = coeffvalues(fitobject);
fitobject = fit(log10(binc(t(n)))', log10(vtrough(t(n)))', 'smoothingspline', 'smoothingparam',.9);
figure(99), clf
loglog(partialdatmerged(datbins2,4),partialdatmerged(datbins2,2)./partialdatmerged(datbins2,5), '.')
hold on
loglog(partialdatmerged(datbins2(ii),4),partialdatmerged(datbins2(ii),2)./partialdatmerged(datbins2(ii),5), 'c.')
...
yest = 10.^(feval(fitobject, log10(partialdatmerged(datbins2,4))'));
