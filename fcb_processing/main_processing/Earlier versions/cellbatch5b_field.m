...
%                beadmatch(sectionnum,8) = NaN; %FIX later to have analvol %pumprate; %0.05;
                ...
                if pecutoff > 1, pecutoff = 10^ceil(log10(pecutoff)); end;
...
%                if intersect(filename(1:4), ['ap21'; 'ap27'; 'ap30'; 'my06'], 'rows')
 %               if setdiff(filename(1:4), ['    '], 'rows') %add in any cases where don't want to use PE cutoff
                    ...
                    a = find(partialdatmerged(datbins2,2) > pecutoff & (partialdatmerged(datbins2,5)./partialdatmerged(datbins2,2) < 50 | partialdatmerged(datbins2,5) < 2e3));                    
 %               else
 ...
 %               end;
                ...
                %a = find(partialdatmerged(datbins2pe,4)./partialdatmerged(datbins2pe,2) < 10 | partialdatmerged(datbins2pe,4) < 3000);                    
                a = find(partialdatmerged(datbins2pe,4).^.8./partialdatmerged(datbins2pe,2) < 1);                    
...
%                junkind = find(pedist > threshhold | (partialdatmerged(datbins2pe,4) >= tempmedian(1)*5 | partialdatmerged(datbins2pe,5) >= tempmedian(2)*5));
                ...
                %tempind = find(partialdatmerged(datbins2nope,4) >  coeff.*partialdatmerged(datbins2nope,5).^power);  %simple fixed line on chl v. ssc for junk cutoff
                tempind = find(temp(:,1) >  coeff.*temp(:,2).^power & temp(:,1) < 1e4);  %simple fixed line on chl v. ssc for junk cutoff, plus only consider chl < 1e4 for smallest euk peak
  ...
  %              threshhold = 8;     %2/25/03 switched back to 10 (from 5) for syn_lab, 6/2/03 changed from 4 to 5; 7/8/03 changed from 5 to 10
                threshhold = 6; %pedist_thre; %changed from 8 to 5 for 2006 and 2007, April 2007
%                junkind = find(pedist > threshhold | (partialdatmerged(datbins2pe,4) >= tempmedian(1)*5 | partialdatmerged(datbins2pe,5) >= tempmedian(2)*5));
