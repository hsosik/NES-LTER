%beadbatch6_field modified from beadbatch5_field to find small beads also
            ...
            if ~isempty(datbinstouse),  %skip cases where no beads in hour
                ...
                if length(beadstocount) > 10,  %otherwise skip
                    ...
                    if length(beadstocount2) > 10,
                        mergedhist = smooth(histc(partialdatmerged(beadstocount2,2:5), bins),4);  %smooth over 2 with 512 ch, and 4 with 1024
                        [junk, modepos2] = max(mergedhist);
                        ...
                        if (bins(modepos2(4))./bins(modepos(4)) > .125) & (bins(modepos2(3))./bins(modepos(3)) < .015) & (length(beadstocount2)/coeffvar(4) < 10) %otherwise not bead mode; CV of 5 for SSC a rough cutoff or scaled by length -> only want for noisy data to exclude 'uniform' dists
%                         if bins(modepos2(4))./bins(modepos(4)) > .125 & bins(modepos2(3))./bins(modepos(3)) < .015 %otherwise not bead mode, likely because too few
                    ...
                    if plotflag,
                        ...
                        if bead2flag, loglog(partialdatmerged(beadstocount2,5),partialdatmerged(beadstocount2,4),'.g', 'markersize', 1), end
                        axis([1 1e6 1 1e6])
                        ...
                        if bead2flag, loglog(partialdatmerged(beadstocount2,5),partialdatmerged(beadstocount2,2),'g.', 'markersize', 1), end
                        axis([1 1e6 1 1e6])
