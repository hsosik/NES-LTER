%% Calculation of attentuation coefficient K for PAR:
...
for filenum=1:numfiles
    ...
    if isempty(data_notes{ii,4}) || ~cellfun('isempty',(regexpi(data_notes(ii,4),'not a cast')))
        ...
        if max(depth) < 7 %may need to change this depending on deployment!
        ...
        else %can proceed with processing the cast:
            ...
            %Plot first and give option to do a split cast:
            ...
            switch ch
                ...
                case 'piecewise'
                    ...
                    K_PAR(filenum).K1=K1;
                    K_PAR(filenum).depth_index1=impr1;
                    K_PAR(filenum).par_index1=ipar1;
                    K_PAR(filenum).stats2=STATS2;
                    K_PAR(filenum).K2=K2;
                ...
                case 'other'
                    ...
                    subplot(1,2,2)
                    plot(K(1)+K(2)*depth(imprB),depth(imprB),'-')
                    text(0.80*diff(xl)+xl(1),0.95*diff(yl)+yl(1),{['K: ' num2str(K(2))];['R2: ' num2str(STATS(1))] })
                    plot(log(edl_PAR),depth(edl_ind),'.'), hold on
                    ...
                    K_PAR(filenum).stats=STATS;
                    K_PAR(filenum).K=K;
                    K_PAR(filenum).depth_index=imprB;
                    K_PAR(filenum).par_index=iparB;
                    K_PAR(filenum).NOTES='manually excluded points';
                    K_PAR(filenum).flag=0;
...
eval(['save ' matsource 'K_PAR_' datafolders{foldernum} 'B.mat K_PAR_' datafolders{foldernum}])
