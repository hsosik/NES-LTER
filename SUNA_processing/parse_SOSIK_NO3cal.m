function cal = parse_SOSIK_NO3cal(cal_file)
...
% TEST
%cal_file = 'C:\Users\jplant\Documents\MATLAB\SOSIK\DATA\SNA1227O.CAL';
...
while ischar(tline)
    ...
    if strncmp(tline,'H',1) % header line so check for header info
        ...
        % SUNA FILE 05/01/2018 JP
        if isempty(cal.CalTemp) & regexp(tline,'^H,T_CAL','once') 
            d_str = regexp(tline,' ','split');
            cal.CalTemp = str2double(d_str{2});
        end
...
% CHECK IF LINES ADDED TO DEFAULT CAL FILE (05/01/2018 JP)
if ~isfield(cal,'pixel_base') || ~isfield(cal,'depth_lag') || ...
        ~isfield(cal,'WL_offset') || ~isfield(cal,'min_fit_WL') || ...
        ~isfield(cal,'max_fit_WL') || ~isfield(cal,'pres_coef')
    disp(['Default cal file appears to be missing some user added ', ...
        'header lines needed to process nitrate data.']);
    disp('Please added the following header lines to the cal file:');
    fprintf(['H,Pixel base,1,,,\n' ...
