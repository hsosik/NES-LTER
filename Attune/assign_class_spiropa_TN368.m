...
%Initialze class vector
   ...
   % end
    ...
    synGL1H2BL3Hoffset = .3; %PE to CHL .4 on RB, .8?
    ...
    minY = max([minY 1200]); %not below trigger level = 1100 for this cruise
    gsyn_11_19 = [0 minY ; 8e3 minY; 8e3 maxY; 0 maxY]; %[Xmin Ymin; Xmax Ymax]
    geuk_15_19 = [eukminX 1100; 8e3 1100; 1100000 1e5; 1100000 1; eukminX 1];
    if (startsWith(filename, 'SPIROPA_TN368_Grazer6')) % ||...
        ...
        geuk_15_19 = [eukminX 1000; 2e4 1000; 1100000 1e5; 1100000 1; eukminX 1];
        gsyn_11_19 = [0 minY ; 8e3 minY; 8e3 maxY; 0 maxY]; %[Xmin Ymin; Xmax Ymax] %maybe only for Grazer4 and after?
    ...
    elseif (startsWith(filename, 'SPIROPA_TN368_Grazer10') || startsWith(filename, 'SPIROPA_TN368_Grazer12')) 
        ...
        geuk_15_19 = [eukminX 1000; 2e4 1000; 1100000 1e5; 1100000 1; eukminX 1];
        gsyn_11_19 = [0 minY ; 1e4 minY; 1e4 maxY; 0 maxY]; %[Xmin Ymin; Xmax Ymax] %maybe only for Grazer4 and after?
    ...
    in_syn = (inpolygon(fcsdatlog(:,npar_synX),fcsdatlog(:,npar_synY),log10(gsyn_11_19(:,1)),log10(gsyn_11_19(:,2))));
    %in_nonsyn_lowPE = fcsdat(:,npar_synY) > minY & fcsdat(:,npar_synY) < maxY & fcsdat(:,19)./fcsdat(:,18) < 2.5 & ~(fcsdat(:,19)<1e4 & fcsdat(:,11)>1e4);
    %in_nonsyn_hiPE = fcsdat(:,npar_synY) > maxY/5 & fcsdat(:,19)./fcsdat(:,18) > 2.5 & fcsdat(:,19)./fcsdat(:,18) < 5;
    in_nonsyn_lowPE = fcsdat(:,npar_synY) > minY & fcsdat(:,npar_synY) < maxY/2 & fcsdat(:,19)./fcsdat(:,18) < 2.5 & ~(fcsdat(:,19)<1e4 & fcsdat(:,11)>1e4);
    in_nonsyn_hiPE = fcsdat(:,npar_synY) > maxY/2 & fcsdat(:,19)./fcsdat(:,18) > 2.5 & fcsdat(:,19)./fcsdat(:,18) < 6;
    ...
    class(class == 0 & fcsdat(:,19)>minY & fcsdat(:,19)<maxY & fcsdatlog(:,15)>fcsdatlog(:,12)*syneukBL3H2SSCHslope+syneukBL3H2SSCHoffset) = 6; %more Syn&Euk coincident
    ...
    if plot_flag
        ...
        line(ax9, xlim(ax9), xlim(ax9)*6) %5
        line(ax9, xlim(ax9), xlim(ax9)*2.5) %2.5
