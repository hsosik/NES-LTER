function [ class , bounds] = assign_class_EN657( fcsdat, fcshdr, plot_flag, filename, QC_flag, startdate )

plot_flag = 0; 

%different gates for different portions of the cruise
if startdate <  7.380795000000000e+05
    phase = 1;
elseif startdate < 7.380806716666666e+05
    phase = 2;
else 
    phase =3; 
end
...
%Initialze class vector
    ...
    gl2_noise_thresh = 9000; %basically synminY 
  

    synGL1A2GL1Hmax = 1; %PE area to height
    synGL1H2BL3Hslope = 1.11; %PE to CHL, ?1.2 with .8 offset?
    synGL1H2BL3Hoffset = 0.35; %PE to CHL .4 on RB, .3 on TN, .8?
    syneukBL3H2SSCHslope = 1.3; %CHL to SSC
    syneukBL3H2SSCHoffset = -1.9; %-.6; %PE to CHL -.8 on TN
    nonsynfactorA = 25; %6
    nonsynfactorB = 15; %2.5

    if phase >= 2
        eukminX = 1400; 
        synXcorners = [4000 24000]; 
        gl2_noise_thresh = 1500; %basically synminY 

        synGL1H2BL3Hslope = 1.01; %PE to CHL, ?1.2 with .8 offset?
    ...
    minY = prctile(fcsdat(in_syn,npar_synY),10)*.2; maxY = prctile(fcsdat(in_syn,npar_synY),90)*10;
    ...
    in_nonsyn_lowPE = fcsdat(:,npar_synY) > minY & fcsdat(:,npar_synY) < maxY/2 & fcsdat(:,npar_synY)./fcsdat(:,17) < nonsynfactorB & ~(fcsdat(:,npar_synY)<1e4);
    in_nonsyn_hiPE = fcsdat(:,npar_synY) >  minY*2 & fcsdat(:,npar_synY)./fcsdat(:,17) > nonsynfactorB & fcsdat(:,npar_synY)./fcsdat(:,17) < nonsynfactorA;
    ...
    class(class ~= 2 & fcsdatlog(:,npar_eukX)<fcsdatlog(:,npar_synX)*syneukBL3H2SSCHslope+syneukBL3H2SSCHoffset) = 0; %more noise
     ...
     if phase == 2 %there are some rogue little low PE cells here
        in_nonsyn_lowPE = class == 0 & fcsdat(:,npar_synX) > 11000 & fcsdat(:,npar_synX) < 180000 & fcsdat(:,npar_synY) > 900 & fcsdat(:,npar_synY) < 15000; 
        class(in_nonsyn_lowPE) = 3;
     end
    ...
    in_nonsyn_hiPE = class ~= 2 & fcsdat(:,npar_synY) >= 7e5;
    class(in_nonsyn_hiPE) = 4;
