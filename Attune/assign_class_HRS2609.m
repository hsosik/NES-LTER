...
%Initialze class vector
    ...
    minX = prctile(fcsdat{in_syn,par_synX},5)*.3; maxX = prctile(fcsdat{in_syn, par_synX}, 90)*10; 
    ...
    frac_coinc = sum(in_syn & (fcsdatlog.(par_synY)<fcsdatlog{:,15}*synGL1H2BL3Hslope+synGL1H2BL3Hoffset))./sum(in_syn);
    while frac_coinc > .03
    ...
    in_nonsyn_lowPE = fcsdat.(par_synY) > minY & fcsdat.(par_synY) < maxY/2 & fcsdat.(par_synY)./fcsdat{:,17} < nonsynfactorB & ~(fcsdat.(par_synY)<1e4 & fcsdat{:,11}>1e4);
    in_nonsyn_hiPE = fcsdat.(par_synY) > maxY/2 & fcsdat.(par_synY)./fcsdat{:,17} > nonsynfactorB & fcsdat.(par_synY)./fcsdat{:,17} < nonsynfactorA;
    ...
    class((fcsdatlog.(par_synY)<fcsdatlog{:,15}*synGL1H2BL3Hslope+synGL1H2BL3Hoffset & fcsdat{:,15}> min(geuk_main_gate(:,1))) & fcsdat.(par_synY)> minY) = 3; %more euks
