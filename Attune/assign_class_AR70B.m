function [ class , bounds] = assign_class_AR70( fcsdat, fcshdr, plot_flag, filename, QC_flag, startdate )

    %This is the cruise that is affected by the messed up clock problem. A copy of the FCS files was 
    %made and Heidi added 5 days, 1 hr and 27 minutes to each file, and then we are running the procesing
    %on the modified files. 
    %This cruise was also during the time period that the blue laser was
    %slightly misaligned so attempting to adjust the Xmin values of Syn to
    %include the Syn that got on the laser edge. 
    %PROBABLY DO NOT REUSE THIS SCRIPT NEXT FALL (AR77-79)
...
%     end
    ...
    minX = prctile(fcsdat(in_syn,npar_synX),5)*.3; maxX = prctile(fcsdat(in_syn, npar_synX), 90)*10; 
