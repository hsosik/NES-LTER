function [ class, bounds ] = assign_class_EN617( fcsdat, fcshdr, plot_flag, filename, QC_flag, startdate )


%different gates for different portions of the cruise
if startdate < 7.372638900810186e+05 %7.372638680555555e+05
    phase = 1 ;
elseif startdate < 7.372648298611111e+05
    phase= 2;
elseif startdate < 7.372659375000000e+05
    phase = 3; 
else 
    phase = 4; 
end
...
%Initialze class vector
    ...
    gl2_noise_thresh = 150; %basically synminY 
  

    synGL1A2GL1Hmax = 4; %PE area to height
    synGL1H2BL3Hslope = 1.4; %PE to CHL
    synGL1H2BL3Hoffset = -1.0; %PE to CHL 
    ...
    if phase ==2
        eukminX = 1400;
        synGL1H2BL3Hslope = 1.3; %PE to CHL
        gl2_noise_thresh = 450; %basically synminY 
    ...
    maxY = prctile(fcsdat(in_syn,npar_synY),95)*10;


    %compare density of two syn minimum threshhold options
    if phase ~= 4; 
    ...
    if sum(islocalmin(f))>0 & f(islocalmin(f))./sum(f) < .015; 
        ...
        minY = minY(1); 
        %sometimes falsely gets upper thresshold
        if minY > 2e3
            minY = gl2_noise_thresh;
        end
    else %if no minimum, go just below trigger threshhold 
        minY = gl2_noise_thresh;
    end
    end
  ...
  if phase == 2
    ...
    if phase ~=2 
    %that worked pretty well let's do it for euks too
    [f, xi] = ksdensity(log10(fcsdat(in_euk & log10(fcsdat(:, npar_eukX))< 3, npar_eukX)));
    if sum(islocalmin(f))>0 & f(islocalmin(f))./sum(f) < .015; 
    ...
    if phase == 4; 
        synGL1H2BL3Hslope = 1.33; %PE to CHL
        synGL1H2BL3Hoffset = -.6; %PE to CHL 
        
        gsyn_main_gate(3,:) = [2000 60000]; 
        gsyn_main_gate(4,:) = [80 1e6]; 
            nonsynfactorB = 3; %2.5
    ...
    in_nonsyn_lowPE = fcsdat(:,npar_synY) > minY & fcsdat(:,npar_synY) < maxY/2 & fcsdat(:,npar_synY)./fcsdat(:,17) < nonsynfactorB & ~(fcsdat(:,npar_synY)<200);
    ...
    %also use FSC-W to see coincidence?
     ...
     if phase == 4
        class(~in_syn & fcsdat(:,npar_synY)> gsyn_main_gate(3,2)) = 4; 
        class(class == 4 & fcsdatlog(:,npar_synY)>fcsdatlog(:,15)*synGL1H2BL3Hslope+synGL1H2BL3Hoffset) = 2; 
     end
    ...
    class(fcsdat(:,npar_eukX) < 100 & fcsdat(:,npar_synY) < 2) = 0; %noise %LAST
