...
%artificial peak in sunlight during hte night or some other light noise at
%night. 
...
function Setup_Cruise_Sliding_Model_Days_Syn_and_Euks(AttuneVolTable)
...
%now we actually make the inputs from the table
    ...
    for dnum = 1:max(Groupnum) %now deal with each overlapping day individually
        ...
        ts = 0; %no limiting division for eukaryotes
