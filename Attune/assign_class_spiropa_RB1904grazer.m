function [ class, bounds ] = assign_class_spiropa_RB1904grazer( fcsdat, fcshdr, plot_flag, filename, QC_flag, startdate )
...
% phase =1 ;
% 
% if  startdate > 7.375629208333333e+05; %17-May-2019 22:06:00, changed to datenum([2019 05 18 08 00 00]) for grazers startdate > 7.3756e+05;
%     phase = 2; 
% end
% if startdate > 7.375658298611111e+05; % 20-May-2019 19:55:00
%     phase =1; %was 3, try putting 1 here. 
% end
% if startdate >  737570; 
%     phase = 1
% end
...
if startsWith(filename, 'SPIROPA_RB1904_Grazer6') || startsWith(filename, 'SPIROPA_RB1904_Grazer7')
    phase = 2;
end

if startsWith(filename, 'SPIROPA_RB1904_Grazer12') 
    phase = 3;
end
...
%Initialze class vector
    ...
    synGL1H2BL3Hslope = 1.2; %PE to CHL, ?1.2 with .8 offset? EP: This line and the one below it affect the line position on plot one,  which designates coincidence. 
    ...
    if phase == 3
        ...
        synminX = 1; 
        synXcorners = [5000 20000];
        gl1_noise_thresh = 5000;%200; 
        syneukBL3H2SSCHslope = 1.5; %CHL to SSC
        syneukBL3H2SSCHoffset = -2.2 %-.6; %PE to CHL
        synGL1H2BL3Hoffset = 0.2;
    end
...
%EP try commenting this out July 13 2023
...
%EP try commenting this out July 13 2023
    ...
    %EP try commenting this out 13 July 2023
