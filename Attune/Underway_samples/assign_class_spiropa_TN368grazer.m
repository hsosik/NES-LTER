...
%Initialze class vector
    ...
    synGL1H2BL3Hslope = 1.2;%2.0; %PE to CHL, ?1.2 with .8 offset? EP: This line and the one below it affect the line position on plot one,  which designates coincidence. 
    synGL1H2BL3Hoffset = .6; %.4; %-1.9; %PE to CHL .4 on RB, .8?
    ...
    minY = prctile(fcsdat(in_syn,19),10)*.3; maxY = prctile(fcsdat(in_syn,19),90)*10; %original line pre EP
    ...
    if (startsWith(filename, 'SPIROPA_TN368_Grazer6')) % ||...
        ...
        maxY = 8e5;
        if eukminX < 1e4
            eukminX = max([1000 eukminX]);%max([1500 eukminX]);
        else 
            eukminX = 1500;
        end
    ...
    elseif (startsWith(filename, 'SPIROPA_TN368_Grazer10') || startsWith(filename, 'SPIROPA_TN368_Grazer12')) 
        ...
        maxY = 1.1e6;
        if eukminX < 1e4
            eukminX = max([1000 eukminX]);%max([1800 eukminX]);
        else 
            eukminX = 800;%1800;
        end    
        geuk_main_gate = [eukminX 1000; 2e4 1000; 1100000 1e5; 1100000 1; eukminX 1];
    ...
    elseif startsWith(filename, 'SPIROPA_TN368_Grazer')
        minY = 3e4; %3e4; minY sets bottom of main Syn box. 
        maxY = 1.1e6; %maxY sets top of main syn box. 
        if eukminX < 1e4
        ...
        geuk_main_gate = [eukminX 1000; 2e4 1000; 1100000 1e5; 1100000 1; eukminX 1];
        %gsyn_main_gate = [0 minY ; 1e4 minY; 1e4 maxY; 0 maxY]; %[Xmin Ymin; Xmax Ymax] %maybe only for Grazer4 and after?
        gsyn_main_gate = [0 minY ; 1e4 minY; 1e4 maxY; 0 maxY];
    end;
...
%     end
   ...
   if sum(in_euk)< 1000 %syneukcioncident only show up when euk concentrations are high
    ...
    in_nonsyn_lowPE = fcsdat(:,npar_synY) > minY & fcsdat(:,npar_synY) < maxY/2 & fcsdat(:,npar_synY)./fcsdat(:,18) < nonsynfactorB & ~(fcsdat(:,npar_synY)<1e4 & fcsdat(:,11)>1e4);
   % in_nonsyn_hiPE = fcsdat(:,npar_synY) > maxY/2 & fcsdat(:,19)./fcsdat(:,18) > nonsynfactorB & fcsdat(:,19)./fcsdat(:,18) < nonsynfactorA;
    ...
    %line on subplot 5 (BL3vSSC) finds not too bright euks with PE (not
    %already syn or euk) **Heidi help EP with this part June 2023, explains
    %why group 3 appears above the lines in plot 8.
