...
% cluster analysis
c = dbscan(log10(fcsdat(:, [ch1 ch2 ch3])), epsilon, minpts);

% identify bead clusters based on high pigment content
pd = NaN(length(unique(c))-1, 1);
for i=1:length(pd)
    pd(i) = sqrt(mean(log10(fcsdat(c==i, ch2)))^2 + mean(log10(fcsdat(c==i, ch3)))^2);
end
