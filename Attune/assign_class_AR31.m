function [ class, bounds ] = assign_class_AR31( fcsdat, fcshdr, plot_flag, filename, QC_flag, startdate )


%different gates for different portions of the cruise
if startdate < 7.373548750000000e+05
    phase = 1; 
else
    phase = 2; 
end
...
%Initialze class vector
    ...
    gl2_noise_thresh = 5000; %basically synminY 
    ...
    syneukBL3H2SSCHoffset = -1.2; %PE to CHL 
    nonsynfactorA = 22; %6
    nonsynfactorB = 4; %2.5

    
    %syn main gate
    gsyn_main_gate = [synminX 16600 ; 400 2000; synXcorners(1) gl2_noise_thresh; synXcorners(2) 110000; synminX synmaxY]; %[Xmin Ymin; Xmax Ymax]
    %euk gate 
    ...
    [f, xi] = ksdensity(log10(fcsdat(in_euk & log10(fcsdat(:, npar_eukX))< 4 & log10(fcsdat(:, npar_eukX))>2, npar_eukX)));
    if sum(islocalmin(f))>0 & f(islocalmin(f))./sum(f) < .015 
        eukminX = 10.^xi(islocalmin(f));
        eukminX = eukminX(1); 
    end
    else
    [f, xi] = ksdensity(log10(fcsdat(fcsdat(:,npar_eukY)<150 & log10(fcsdat(:, npar_eukX))< 3.6 & log10(fcsdat(:, npar_eukX))>2.2, npar_eukX)));
    if sum(islocalmin(f))>0 & f(islocalmin(f))./sum(f) < .015 
        eukminX = 10.^xi(islocalmin(f));
        eukminX = eukminX(1); 
    end
    ...
    class( in_syn & fcsdat(:, npar_synX) > 700)= 5; %after getting coincident, rule out larger syn. 
    ...
    class(in_nonsyn_hiPE) = 4;
    class( ~in_syn & fcsdat(:,npar_synY) > 11000) = 4; 


    %some of the coincident cells are low PE euks.
    PE_factor = 1.3; 
    in_nonsyn_lowPE = class == 5 & fcsdat(:,18)./fcsdat(:,npar_synY) > PE_factor; 
    class(in_nonsyn_lowPE) = 3; 



    class(class == 1 & fcsdat(:,npar_eukX) < 180)  = 0; %noise %LAST
